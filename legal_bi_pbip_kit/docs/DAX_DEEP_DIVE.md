...
The full measure catalog lives in [`DAX_MEASURES.dax`](DAX_MEASURES.dax) and is emitted into `model.bim` by `scripts/generate_legal_bi_pbip.py`. This document picks four measures and explains the design choices behind them - the kind of conversation a Senior BIE interview tends to probe.
