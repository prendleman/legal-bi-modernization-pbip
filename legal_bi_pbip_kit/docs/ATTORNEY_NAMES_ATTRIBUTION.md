...
`legal_bi_pbip_kit/scripts/report_assets/sidley_public_attorney_names.json`
...
`generate_legal_bi_pbip.py` loads the JSON when present. Rows `1 … len(display_names)` use each name once; additional synthetic attorney rows append `(demo roster NNN)` so `AttorneyName` stays unique for `AttorneyKey`.
