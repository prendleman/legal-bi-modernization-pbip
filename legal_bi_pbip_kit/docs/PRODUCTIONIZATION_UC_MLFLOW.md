...
The demo uses **catalog/schema/volume defaults** (`sidley_demo` / `gold` / `landing`) so it can stand up cleanly in any dev workspace; override with `--catalog`, `--schema`, `--volume` on `generate_legal_bi_pbip.py`.
