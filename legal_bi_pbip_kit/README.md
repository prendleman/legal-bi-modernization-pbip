...
A fast-start, source-control-friendly Power BI Project (PBIP) **built as a role-aligned portfolio demo for a Senior Business Intelligence Engineer interview process**. It demonstrates:
...
This repo ships honest **data previews** of every page, generated by Python from the
same synthetic gold CSVs the PBIP report binds to. They are clearly labeled as
previews, not Desktop screenshots, so reviewers can skim the data shape without
launching Power BI:

[![Executive Overview preview](../previews/pages/01_executive_overview.png)](../previews/pages/01_executive_overview.png)

All eight pages plus a single-page HTML mockup live under
[`../previews/`](../previews/README.md). Regenerate with:

```powershell
pip install -r requirements-preview.txt
py scripts\render_page_previews.py     # 8 PNGs into ../previews/pages/
py scripts\build_html_mockup.py        # ../previews/dashboard_mockup.html
```

See [`../previews/README.md`](../previews/README.md) for what these are, what they
cannot show (RLS, slicer sync, bookmarks, drillthrough), and how they relate to the
PBIP build.
...
If your prompt is already `...\legal_bi_pbip_kit>`, run that command only. A second `cd legal_bi_pbip_kit` fails because it looks for a nested folder that does not exist. From repo root `...\sid` you can use `python scripts\smoke_pbip.py` instead (see `scripts/smoke_pbip.py` one level above this package).
...
```text
py scripts\generate_legal_bi_pbip.py [--out PATH] [--seed N] [--rows N]
...
Open the generated **`output\Sidley_BI_Modernization_Demo\Sidley_BI_Modernization_Demo.pbip`** (or re-run `python scripts\smoke_pbip.py`) so new pages match `scripts/generate_legal_bi_pbip.py`.
