...
4. **Unity Catalog catalog + schema + volume** matching the bundle variables (defaults: `sidley_demo` / `gold` / `landing`). The demo's `--databricks` flag in `generate_legal_bi_pbip.py` provisions these idempotently.
...
copy legal_bi_pbip_kit\databricks\asset_bundle\databricks.template.yml .\databricks.yml
...
| --- | --- |
| `./legal_bi_pbip_kit/docs/databricks_notebook` | `legal_bi_pbip_kit/docs/databricks_notebook.py` (`# Databricks notebook source` style) |

Two ways to make the path resolvable in the workspace:

- **Repos** (recommended): clone this repo as a Databricks Repo so `/Repos/<you>/sid/legal_bi_pbip_kit/docs/databricks_notebook` matches the bundle's relative path. The bundle CLI normalizes the prefix.
...
| `catalog` | `sidley_demo` | Matches `generate_legal_bi_pbip.py --catalog`. |
...
```powershell
pip install -r legal_bi_pbip_kit\requirements.txt
