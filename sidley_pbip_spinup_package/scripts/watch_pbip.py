"""Watch generator sources and re-run generate_sidley_pbip.py when files change.
...
def _run_generate(forward_argv: list[str]) -> int:
    cmd = [sys.executable, str(SCRIPTS / "generate_sidley_pbip.py"), *forward_argv]
...
def main() -> int:
    ...
    try:
        while True:
            ...
            last = _fingerprint()
            if code != 0:
                print(f"generate_sidley_pbip exited with {code} (watching continues)\n", flush=True)
