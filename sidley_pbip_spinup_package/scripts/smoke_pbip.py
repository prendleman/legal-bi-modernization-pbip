...
def _page_order_len_from_generator(gen_path: Path) -> int:
    """How many report tabs `generate_sidley_pbip` should emit (length of PAGE_NAMES)."""
...
GEN = PACKAGE_ROOT / "scripts" / "generate_sidley_pbip.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--skip-gen",
        action="store_true",
        help="Do not run generate_sidley_pbip.py; only validate existing output/ (faster local check).",
    )
    parser.add_argument(
        "--interview",
        action="store_true",
    ...
    if not GEN.is_file():
        ...
        print("  cd sidley_pbip_spinup_package", file=sys.stderr)
