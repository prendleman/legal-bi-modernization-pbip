...
After `py scripts\generate_sidley_pbip.py`, open the PBIP in Power BI Desktop once. If a visual shows a field-well error, the query role name may differ slightly by Desktop version — fix in Desktop, **Save**, then diff the emitted `visual.json` and port the change into `scripts/pbir_visuals.py`.
